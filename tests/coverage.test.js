...
//
// task-15, 2026-09-17: 하한선을 64 → 63으로 다시 1 내림. 전북대학교(jbnu)의
// '음악과'는 순수 서양 클래식 전공(성악/피아노/관현악/작곡)이며 실용음악
// 관련 내용이 학과소개·연혁·교육특성 어디에도 없음을 직접 확인했다(music.jbnu.ac.kr).
// "실용음악작곡"이라는 문구는 완전히 다른 학과(한국음악학과, 국악)의 세부전공
// 명칭일 뿐이었다. bucheon과 같은 패턴 — 모집이 중단된 것이 아니라 애초에
// 이 앱의 스코프(대중음악 실기 전형)에 속한 적이 없는 학과가 잘못 등재됐다.
T.test('학교 수가 검증된 하한선(63) 아래로 떨어지지 않는다', function () {
  T.assert(PM.SNAPSHOT.schools.length >= 63,
    '현재 ' + PM.SNAPSHOT.schools.length + '곳 — 검증된 학교가 하나 이상 유실됨(하한선 63)');
});
