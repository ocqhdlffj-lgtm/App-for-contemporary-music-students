...
- 전국 63개교, `확인됨` 40곳 · `부분확인` 12곳 · `미확인` 11곳
- `미확인` 11곳 중 10곳은 학과가 실재(했)지만 최근 모집단위에서 빠졌거나
  스코프(대중음악 실기 전형)에서 벗어난 것으로 조사된 곳이다 — 삭제하지 않고
  `prepPoints`에 경위를 남겨뒀다. 학교 측 확인 후 존치 여부를 재검토할 것.
